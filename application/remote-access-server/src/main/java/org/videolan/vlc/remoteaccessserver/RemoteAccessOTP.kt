...
object RemoteAccessOTP {
    ...
    private val codes = ArrayList<org.videolan.vlc.remoteaccessserver.OTPCode>()
     ...
     */
    private fun generateOTPCode(): org.videolan.vlc.remoteaccessserver.OTPCode {
        val code = org.videolan.vlc.remoteaccessserver.RemoteAccessOTP.generateCode()
        val otpCode = org.videolan.vlc.remoteaccessserver.OTPCode(code, SecretGenerator.generateRandomString(), System.currentTimeMillis() + 60000)
        org.videolan.vlc.remoteaccessserver.RemoteAccessOTP.codes.add(otpCode)
        return otpCode
    }
    ...
    fun verifyCode(appContext: Context, saltedCode: String): Boolean {
        org.videolan.vlc.remoteaccessserver.RemoteAccessOTP.codes.forEach {
            if (CypherUtils.hash(it.code + it.challenge) == saltedCode && System.currentTimeMillis() < it.expiration) {
                ...
                org.videolan.vlc.remoteaccessserver.RemoteAccessOTP.codes.remove(it)
                return true
            }
        }
        return false
     ...
     */
    fun getFirstValidCode(appContext: Context): org.videolan.vlc.remoteaccessserver.OTPCode {
        val toRemove = ArrayList<Int>()
        org.videolan.vlc.remoteaccessserver.RemoteAccessOTP.codes.forEachIndexed() { index, code ->
        ...
        toRemove.forEach {
            org.videolan.vlc.remoteaccessserver.RemoteAccessOTP.codes.removeAt(it)
        }
        val code = org.videolan.vlc.remoteaccessserver.RemoteAccessOTP.generateOTPCode()
    ...
    fun removeCodeWithChallenge(challenge: String) {
        org.videolan.vlc.remoteaccessserver.RemoteAccessOTP.codes.remove(org.videolan.vlc.remoteaccessserver.RemoteAccessOTP.codes.find { challenge == it.challenge })
    }
}
