...
class RemoteAccessOnboardingOtpFragment : RemoteAccessOnboardingFragment() {
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        deviceOTP.text = org.videolan.vlc.remoteaccessserver.RemoteAccessOTP.generateCode()
        ...
        animSet.doOnEnd {
            lifecycleScope.launch {
                viewLifecycleOwner.repeatOnLifecycle(Lifecycle.State.STARTED) {
                    delay(400)
                    val text = if (iteration % 2 == 0) deviceOTP.text else org.videolan.vlc.remoteaccessserver.RemoteAccessOTP.generateCode()
