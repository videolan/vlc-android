...
class MediaBrowserTvFragment : BaseBrowserTvFragment<MediaLibraryItem>() {
    override fun provideAdapter(eventsHandler: IEventsHandler<MediaLibraryItem>, itemSize: Int): TvItemAdapter {
        return MediaTvItemAdapter(
            when ((viewModel as MediaBrowserViewModel).category) {
    ...
    companion object {
        fun newInstance(type: Long, item: MediaLibraryItem?) = MediaBrowserTvFragment().apply {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        (viewModel.provider as MedialibraryProvider<*>).pagedList.observe(this) { items ->
            ...
            //headers
            val nbColumns =
                if ((viewModel as MediaBrowserViewModel).sort == Medialibrary.SORT_ALPHA || (viewModel as MediaBrowserViewModel).sort == Medialibrary.SORT_DEFAULT) 9 else 1
    ...
    override fun onClick(v: View, position: Int, item: MediaLibraryItem) {
        lifecycleScope.launchWhenStarted {
            if ((viewModel as MediaBrowserViewModel).category == CATEGORY_VIDEOS && !Settings.getInstance(requireContext())
                    .getBoolean(FORCE_PLAY_ALL_VIDEO, Settings.tvUI)
            ) {
                TvUtil.playMedia(requireActivity(), item as MediaWrapper)
            } else if ((viewModel as MediaBrowserViewModel).category == CATEGORY_SONGS && !Settings.getInstance(requireContext())
                    .getBoolean(FORCE_PLAY_ALL_AUDIO, Settings.tvUI)
            ) {
                TvUtil.playMedia(requireActivity(), item as MediaWrapper)
            } else {
