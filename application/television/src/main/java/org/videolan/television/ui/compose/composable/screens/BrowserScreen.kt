...
@Composable
fun BrowserScreen(item: MediaLibraryItem, viewModel: MainActivityViewModel = viewModel()) {
    ...
    Scaffold(
        ...
        Box {
            BrowserScreenContent(Modifier.padding(contentPadding), item)
            DisplaySettings()
        }
    }
...
@Composable
fun BrowserScreenContent(modifier: Modifier, item: MediaLibraryItem) {
    Column(modifier
        .fillMaxHeight()
        ) {
...
//                    .focusRequester(focusRequester = focusRequester),
            ...
            Text(text = stringResource(R.string.talkback_folder, item.title))
        }
        Row(modifier) {
            AudioPlayer()
            BrowserList(item, modifier = Modifier.padding(
                top = 16.dp,
                start = 24.dp,
                end = 24.dp
            ))
